--- title: "Optimizal 15 puzzle solver" date: 2026-05-10 tags: ["rust",
"puzzle", "solver"] draft: true ---

Since I was young I always enjoyed puzzle, at two levels: 1) Solving them
...
and Iterative Depth. Even a bad algorithm, assuming you have enough time, will
solve the Cube. The fun is in solving it as fast as possible. Here there are
two approachs: 1) Improving the search heuristics, like using group theory and
...
was written by hand, and a significant time was spend just on debugging. It was
...
from scratch without human input, I got curious with a posibility of writing a
puzzle solved without doing a single line of code. This would remove the fun of
writing the code and debugging and all the pride in the craftsmanshift. But in
exchange it would allow to focus exclusivelly in the thinking process on how to
...
The solve hosted version quickly became stuck when implementating the data
...
was extremely slow. At this point it was contantly getting lost and becaming
stuck. Then I gave up on it.

With Claude Code, as expected of a comercial product using one of the most
...
test multiple hypothesis. With the agent it was extremely easy to test new
approachs. It built caches for removing the cost of computing transposition
...
something from scratch. Since this was something way more complex that a SaaS
project, it was actually fun. It was like being a professor and guiding
students and technitians on a research project. It replaces the fun of writting
the code, with the fun of thinking how to make it faster. The iteration loop
for thinking of new ways to optimizate was significantly reduced, where I could
do a pretty respectable solved in an afternoon. My previous rubiks cube
...
emails while it crunched a bunch a numbers, and then I saw multiple emails
about running out of balance and it being automatically refiled. I spent $380
in an afternoon vibecoding something. Here where I live this is the 70th
percentile for per capita incoming in the poorest states.
